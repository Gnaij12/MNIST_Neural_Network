...
def testperceptron(A,wlist,blist,x,y):
    ...
    for i in range(len(x)):
        alist = pnetAlist(A,wlist,blist,x[i])
        out = alist[len(alist)-1]
        mis += findmisclassified(out,y[i])
    return mis,len(x)
...
# print(wlist[2])
# print(blist)
x,y = takefromfile("mnist_test.csv")
mis,total = testperceptron(sigmoid,wlist,blist,x,y)
print("Percent wrong: %s" %(mis/total*100))